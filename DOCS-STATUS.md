...
| `exyliacapture` | `ExyliaCapture` | 1.1.1 | `2f033d0` 2026-09-23 | Current — reviewed for 1.1.1: the eighth type, Bounty, with its settings, tokens and placeholders; the three API events (`CaptureStartEvent`, `ZoneCaptureEvent`, `CaptureEndEvent`) and `openMenu`; `config.yml` version 3 with `event-visuals` keyed by type and the per-type sidebars; schedules on the ExyliaLib module with `every`, gates and the ExyliaLib timezone; ExyliaStaff players on duty ignored; full-inventory rewards queued. Corrected: the leaderboard keys, the boss-bar `colour`/`overlay` keys and the multi-event `line1-format`/`line2-format`. The repository's `DOCUMENTATION.md` is stale and was not used. |
...
| `exyliakilleffect` | `ExyliaKillEffect` | 1.5.0 | `e50a88a` 2026-09-16 | Current — reviewed for 1.5.0: the 110 effects in five categories (thirty in meme, ten of them new: `SIX_SEVEN`, `PLATE_SMASH`, `ROTTEN_EGG`, `COFFIN_DANCE`, `SKIBIDI`, `METAL_PIPE`, `TO_BE_CONTINUED`, `EJECTED`, `VINE_BOOM`, `AURA_LOSS`), `effects.ground-snap` playing a mid-air kill on the floor under it, the `trial` effect a player without one is shown now and then, `showcase` locations that loop effects cast with nearby players' skins and their `/killeffectsadmin showcase` subcommands, and `player`, `keys` and `unlocks` writing to an offline player's row. Every effect (meme, void, cataclysm, lovestruck, cosmic) is built on the choreographed `[RAGDOLL]` step with `keys:`, `{killer}`/`{crowd}` bodies, `then:`, props, strings and chains, the `[PIXELS]` shape, the sound volume cap, the play/menu/unlock/key API and `KillEffectPlayEvent`, crate keys as an item, the reward queue with `rewards-claimed`, and crate blocks binding when their world loads. |
