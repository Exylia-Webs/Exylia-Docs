...
| `RESTING` | The drill ran its course and the player stays in the arena. | Plays the drill again. |
...
1. The `session-complete` effect plays ("TIME", "Punch to play again").
...
The player then rests for `training.rest-seconds` (`5` by default). A punch in that window, the
**Play again** button on the summary or the hotbar replays the drill; **Change drill** starts another one.
The time running out sends them home, but only if they are idle:
