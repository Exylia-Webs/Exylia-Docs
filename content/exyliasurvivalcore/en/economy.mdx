...
<Callout type="warning" title="`vault` is not a stored currency">
With the default left at `vault`, everything that only a stored currency can do comes back empty
when no currency is named: `/baltop`, `/economy top`, `/economy history`, and the top placeholders
without a currency id. `/pay` without a currency also skips the stored currency's rules — its
permission, the transfers switch, the minimum and the tax — and its ledger line reads `vault` rather
than `pay`.

Setting `default-currency: dollars` (or whichever stored currency the server runs on) makes all of
them read that currency directly.
</Callout>
...
The leaderboards are this module's; the balances are ExyliaLib's, because they work for any currency.
...
| `%exylialib_economy_balance_<currency>%` | The viewer's balance, formatted |
| `%exylialib_economy_compact_<currency>%` | The same, short |
| `%exylialib_economy_raw_<currency>%` | The number alone |
| `%exylialib_economy_name_<currency>%` | The currency's plural name |
| `%exylialib_economy_symbol_<currency>%` | Its symbol |

Leaving the currency off reads the default currency — which, left at `vault`, has no leaderboard.
